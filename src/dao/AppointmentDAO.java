...
public class AppointmentDAO{
     ...
     * The params (key,value) options are: (patient,patientID), (doctor,doctorID), (specialization,specializationID),
     ...
     */
    public boolean update(Appointment appointment, Patient patient) {
        Optional<Appointment> appo = get(appointment.getID());
        if(appo.isPresent()){
            if(appo.get().getPatient() != null){
                return false;
            }
        }
    ...
    /**
     * Generates appointment from result set from database
     * It is an enhanced version because it uses a better version of queries, that allow to create appointment object
     * without calling for doctor, patient etc. DAOs, which makes creating many appointments objects very slow.
     * @param rs Result set that contains appointment fields from database
     * @return An appropriate appointment object
     * @throws SQLException
     */
